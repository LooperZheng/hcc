...
void initAns(int netNodeOutDegree[], int initAns[],int netNodeNum)
{
    ...
    for(int i=0; i<netNodeNum*0.05;i++)
    {
...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num,char * filename)
{
	...
	for(int i=4;i<4+edgeNum;i++)//读取网络节点边的费用和带宽
	{
        ...
        //双向图转换为单向图
        mcmf.addedge(netIDtoMyID[b],netIDtoMyID[b]+edgeNum,c,d);
        mcmf.addedge(netIDtoMyID[b]+edgeNum,netIDtoMyID[a],c,d);
        //统计每个节点的出度
